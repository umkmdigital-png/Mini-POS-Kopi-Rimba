...
function calcTotal() {
    ...
    const totalJualSesi = sessionOrders.reduce((a, o) => a + o.subtotal, 0);
    const totalCupSesi = sessionOrders.reduce((a, o) => a + o.totalCup, 0);
    
    const totalBrutoAll = totalJualSesi + currentBruto;
    const totalCupAll = totalCupSesi + currentCups;
    const totalKeluar = expenses.reduce((a, b) => a + b.price, 0);
    const qris = cleanNum(document.getElementById('p_qris')?.value);
    const netto = totalBrutoAll - totalKeluar - qris;
    ...
    updateUI('total-cup-val', totalCupAll + " Cup");
    updateUI('total-tunai-val', "Rp " + netto.toLocaleString('id-ID'));
...
function confirmOrder() {
    ...
    const custName = custInput.value.trim() || "Pelanggan"; // Default nama customer
    ...
    // Simpan order dengan nama customer
    sessionOrders.push({ 
        id: orderCounter, 
        time: fmtTime(now()), 
        customer: custName, 
    ...
    cart = {};
    custInput.value = ""; // Bersihkan input nama setelah confirm
    updateAll();
    closeCartModal();
    showToast(`Pesanan #${orderCounter} (${custName}) dikonfirmasi!`);
}
    ...
    [...sessionOrders].reverse().forEach(o => {
        list.innerHTML += `
            <div class="riwayat-row" style="border-bottom:1px solid #eee; padding:10px 0">
                ...
                <div style="font-size:11px; color:#999; margin-bottom:4px;">🕒 ${o.time}</div>
...
async function checkout() {
    ...
    const totalJual = sessionOrders.reduce((a, o) => a + o.subtotal, 0);
    const totalCup = sessionOrders.reduce((a, o) => a + o.totalCup, 0);
    const totalKeluar = expenses.reduce((a, b) => a + b.price, 0);
    const qris = cleanNum(document.getElementById('p_qris')?.value);
    const netto = totalJual - totalKeluar - qris;

    // Rekap Detail Item Terjual
    let itemRekap = {};
    ...
    const payload = {
        ...
        detailPengeluaran: detailKeluarSheet, qris, netto,
        stok: `Sisa Cup: ${sisaCup}, Bahan: ${sisaBahan}`,
        detailTerjual: detailTerjualSheet // Tambahan data sheet opsional
    };
    
    await sendToSheet(payload);

    let teks = `*LAPORAN KOPI RIMBA*\n👤 ${nama}\n🏠 ${outlet || '-'}\n📅 ${fmtDate(nowTime)}\n━━━━━━━━━━━━\n\n`;
    teks += `*RINGKASAN JUAL:*\n🥤 Total: ${totalCup} Cup\n💰 Bruto: Rp ${totalJual.toLocaleString('id-ID')}\n\n`;
    teks += `*DETAIL MENU TERJUAL:*\n${detailTerjualTeks}\n━━━━━━━━━━━━\n\n`;
    teks += `*RINCIAN PENGELUARAN:*\n${expenses.length > 0 ? detailKeluarTeks + `\n> *Total:* Rp ${totalKeluar.toLocaleString('id-ID')}\n\n` : "• Tidak ada pengeluaran\n\n"}`;
    teks += `*PEMBAYARAN:*\n📱 QRIS: Rp ${qris.toLocaleString('id-ID')}\n💵 *SETORAN TUNAI: Rp ${netto.toLocaleString('id-ID')}*\n━━━━━━━━━━━━\n\n`;
...
function resetSemuaData() {
    ...
    const fields = ['p_qris', 'stok_cup', 'stok_bahan', 'stok_note', 'exp-name', 'exp-price', 'cart-customer-name'];
